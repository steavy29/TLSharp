...
{
    public class MtProtoConnection : IDisposable
    {
        ...
        public MtProtoConnection(TcpTransport transport, Session session)
        {
        ...
        {
            if (!_runningRequests.ContainsKey(messageId))
                return true;

            var request = _runningRequests[messageId].Item1;
        ...
        private bool HandleBadMsgNotification(long messageId, int sequence, BinaryReader messageReader)
        {
            ...
            _runningRequests[messageId].Item1.OnError(errorCode, message);
            /*
